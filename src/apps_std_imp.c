...
int fopen_from_dirlist(const char *dirList, const char *delim, 
  ...
  int nErr = AEE_SUCCESS;
  char *absName = NULL, *pos = NULL;
  /* dirName points into the dirList string (which is const char *).
   * Declaring it as char * causes a -Wincompatible-pointer-types-discards-
   * qualifiers error (promoted to -Werror by clang) at the assignment
   * dirName = dirList below.  Use const char * instead. */
